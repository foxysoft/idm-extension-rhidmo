...
public class TestUtlGetScriptNamesOfTask {
  ...
  public void onlyMain() throws Exception {
    ...
    Mockito.when(mockTask.getParameter(eq(EVENT))).thenReturn(MAIN);
    List<PackageScript> act = Rhidmo.getUtl().getScriptNamesOfTask(mockTask, EVENT);
  ...
  public void mainWithOneRequired() throws Exception {
    ...
    Task mockTask = Mockito.mock(Task.class);
    Mockito.when(mockTask.getParameter(eq(EVENT))).thenReturn(MAIN);
    Mockito.when(mockTask.getParameter(eq(REQ1_KEY))).thenReturn(REQ1_VALUE);
