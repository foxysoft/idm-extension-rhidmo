...
public class TestUtl {
  ...
  class MockGlobalFunctions extends ScriptableObject {
    ...
    public void rhidmo_someMethod(String s, int i) {}
  ...
  public void testRegisterGlobalFunctions() throws Exception {
    ...
    Rhidmo.getUtl()
        .registerPublicMethodsInScope(
            MockGlobalFunctions.class, mockScope, mockGlobalFunctionScope);
    Mockito.verify(mockScope)
        .put(
            eq("rhidmo_someMethod"),
            eq(mockScope),
            argThat(
                new ArgumentMatcher<FunctionObject>() {
                  @Override
                  public boolean matches(FunctionObject fo) {
                    return fo.getFunctionName().equals("rhidmo_someMethod") && fo.getArity() == 2;
                  }
                }));
  }
}
