...
public class Init {
  ...
  public static synchronized void doInit() throws Exception {
    ...
    if (!g_initialized) {
      ...
      ClassLoader jmxClassLoader = idmFactory.getClass().getClassLoader();
      ...
      ClassLoader combinedClassLoader =
          new SequentialDelegationClassLoader(contextClassLoader, jmxClassLoader);

      // Get adapter base class from JMX class loader
      Class<?> baseClass =
          Class.forName("com.sap.idm.extension.TaskProcessingAdapter", true, combinedClassLoader);
      ...
      Class<?> subClass =
          new ByteBuddy()
              ...
              .intercept(MethodDelegation.to(TaskProcessingStatic.class))
      ...
      Class<?> interfaceClass =
          Class.forName("com.sap.idm.extension.ITaskProcessing", true, jmxClassLoader);
      idmFactory
          .getClass()
          .getMethod("registerTaskProcessingInterface", new Class<?>[] {interfaceClass})
          .invoke(idmFactory, new Object[] {subClass.newInstance()});
      LOG.debug(M + "subClass registered");

      ContextFactory.getGlobal().initApplicationClassLoader(combinedClassLoader);
