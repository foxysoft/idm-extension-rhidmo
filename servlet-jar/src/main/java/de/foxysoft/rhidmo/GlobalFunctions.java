...
public class GlobalFunctions extends ScriptableObject {
  ...
  /**
   * Create a new GlobalFunctions instance from task and configuration. This constructor will create
   * a file-based IniKeyStorageProvider from the value of property de.foxysoft.idm.crypt.keyfile
   * contained in configuration.getProperties().
   *
   * <p>References to both task and props will be held throughout the lifetime of this
  ...
  private static final Log APPL_LOG = Log.get("de.foxysoft.rhidmo.Application");
  ...
  public String uSelect(String sqlStatement, Object rowSeparator, Object columnSeparator) {
    final String M = "uSelect: ";
    LOG.debug(
        M + "Entering sqlStatement={}, rowSeparator={}, columnSeparator={}",
        sqlStatement,
        rowSeparator,
        columnSeparator);

    String result = null;
    if (rowSeparator == null || Undefined.instance == rowSeparator || "".equals(rowSeparator)) {
      rowSeparator = "!!";
      LOG.debug(M + "Using default rowSeparator={}", rowSeparator);
    }
    ...
    if (columnSeparator == null
      ...
      columnSeparator = "|";
      LOG.debug(M + "Using default columnSeparator={}", columnSeparator);
    }
  ...
  public String uFromHex(String hexString, String characterEncoding) {
    final String M = "uFromHex: ";
    LOG.debug(M + "Entering hexString={}, characterEncoding={}", hexString, characterEncoding);

    String result = "";
    if (Undefined.instance == hexString || null == hexString || "".equals(hexString)) {
      hexString = "";
      LOG.debug(M + "Defaulted hexString={}", hexString);
    }
    ...
    if (Undefined.instance == characterEncoding
      ...
      LOG.debug(M + "Defaulted characterEncoding={}", characterEncoding);
    }

    int offset = hexString.startsWith("{HEX}") ? "{HEX}".length() : 0;
    int dataLen = hexString.length() - offset;
    byte[] data = new byte[dataLen / 2];
    for (int i = 0; i < dataLen; i += 2) {
      data[i / 2] =
          (byte)
              ((Character.digit(hexString.charAt(i + offset), 16) << 4)
                  + Character.digit(hexString.charAt(i + offset + 1), 16));
    }
  ...
  private SecretKey determineSecretKey(byte[] binaryKey, String keyType, int keySize)
      throws Exception {
    final String M = "determineSecretKey: ";

    LOG.debug(
        M + "Keysize = {}, AES Maximum {}", keySize, Cipher.getMaxAllowedKeyLength("AES") / 8);
    if (keyType == "AES") {
  ...
  public String uDecrypt(String cipherText, String providedKey, Object charEncoding) {
    ...
    try {
      ...
      SecretKey cipherKey =
          determineSecretKey(
              key, this.myKeyStorage.getSecretKeyName(), this.myKeyStorage.getCurrentKeySize());

      AlgorithmParameters algParams =
          AlgorithmParameters.getInstance(this.myKeyStorage.getSecretKeyName());
    ...
    if (charEncoding == null || Undefined.instance == charEncoding || "".equals(charEncoding)) {
  ...
  public String uEncrypt(
      String clearText, String providedAlgorithm, String providedKey, Object charEncoding) {
    final String M = "uEncrypt: ";
    LOG.debug(M + "Entering. Algorithm = {}, charEncoding = {}", providedAlgorithm, charEncoding);

    try {
      if (providedAlgorithm instanceof String
          && providedAlgorithm != null
          && !"".equals(providedAlgorithm)) {
      ...
      SecretKey key =
          determineSecretKey(
              ...
              this.myKeyStorage.getSecretKeyName(),
              this.myKeyStorage.getCurrentKeySize());
      ...
      return this.myKeyStorage.getAlgorithmDescription()
          ...
          + ":"
          + DatatypeConverter.printHexBinary(cp.getIV()).toLowerCase()
          + "-"
  ...
  public String uIS_GetValue(int mskey, int idStore, String attrName) {
    ...
    try {
      ...
      // given.
      if (idStore == 0) {
        PreparedStatement getIdStoreStatement =
            con.prepareStatement("Select max(is_id) from idmv_value_basic where mskey = ?");
      ...
      PreparedStatement getAttributeStatement =
      ...
      String sqlStatement =
          "select avalue from idmv_value_basic where mskey = ? and attrname = ?  and is_id = ?";
  ...
  public int uGetIDStore() {
    ...
    try {
      int taskId =
          ((Integer) this.myTask.getClass().getMethod("getID").invoke(this.myTask)).intValue();
  ...
  public String uIS_nGetValues(int mskey, String attrName, Object separator) {
    final String M = "uIS_nGetValues: ";
    LOG.debug(M + "Entering = {}", mskey, attrName, separator);

    String separator2Use = "";
    if (separator == null || Undefined.instance == separator || "".equals(separator)) {
      separator2Use = "!!";
    ...
    try {
      con = myUtl.getConnection();
      PreparedStatement getValuesStatement =
          con.prepareStatement(
  ...
  public String uSendSMTPMessage(
      String sender,
      String recipients,
      String subject,
      String messageOrFilename,
      String smtpHost,
      Object contentType,
      Object port,
      Object attachment,
    ...
    if (contentType == null || Undefined.instance == contentType || "".equals(contentType)) {
    ...
    try {
      ...
      if (recipientList == null) {
        ...
        } else emailMessage.setRecipients(RecipientType.TO, recipients);
      } else {
        for (int i = 0; i < recipientList.length; i++) {
          ...
          } else emailMessage.addRecipients(RecipientType.TO, recipientList[i]);
        }
      }
