...
public class Utl {
  ...
  public Connection getConnection() throws Exception {
    ...
    DataSource datasource = (DataSource) ic.lookup("jdbc/RHIDMO_RT");
    Connection result = datasource.getConnection();
    LOG.debug(M + "Returning {}", result);
    return result;
  }

  public void registerPublicMethodsInScope(Class<?> c, Scriptable parentScope, Scriptable scope) {
    ...
    for (int i = 0; i < methods.length; ++i) {
      ...
      if (Modifier.isPublic(modifiers)
        ...
        functionObject = new MyFunctionObject(methodName, methods[i], scope);
        parentScope.put(methodName, parentScope, functionObject);
  ...
  public Function execScriptsInScope(
      List<PackageScript> packageScripts, Scriptable scope, Context context, int taskId)
      throws Exception {
    ...
    for (int i = 0; i < packageScripts.size(); ++i) {
      ...
      if (psi.getScriptSource() != null) {
        Script script = context.compileString(psi.getScriptSource(), psi.getScriptName(), 1, null);

        script.exec(context, scope);
      } // if
      else {
        LOG.warn(
            M + "Script {} referenced by parameters of task {} does not exist",
            psi.getScriptName(),
            taskId);
      }
    } // for (int i = 0; i < packageScripts.size(); ++i) {

    String mainScriptName = packageScripts.get(0).getScriptName();
    Object resultObj = scope.get(mainScriptName, scope);

    if (UniqueTag.NOT_FOUND.equals(resultObj)) {
      throw new ErrorException(
          "Script " + mainScriptName + " doesn't define any property named " + mainScriptName);
    }
  ...
  /**
   * <div>Shallow copy package scripts with qualified names (package name + script name) from all to
   * qualifiedNames.</div> <div>Shallow copy package scripts with simple names (script name only)
   * from all to simpleNames.</div>
   *
   ...
   */
  protected void filterPackageScripts(
      List<PackageScript> all,
      List<PackageScript> qualifiedNames,
      List<PackageScript> simpleNames) {
  ...
  /**
   * <div>Find the script with the given scriptName/packageName combination in packageScripts using
   * linear search and set script source to the BASE64 decoded version of encodedScript. If
   * encodedScript is not BASE64 encoded (this is used by some unit tests), use its content as
   * script source directly.</div> <div>If scriptName/packageName doesn't exist in packageScripts,
   * nothing will be changed.</div>
   *
   ...
   */
  protected void setScriptSource(
    ...
    if (encodedScript.startsWith("{B64}")) {
      decodedScript =
          new String(Base64.decodeBase64(encodedScript.substring("{B64}".length())), "UTF-8");
      LOG.debug(M + "decodedScript = {}", decodedScript);
    } else {
  ...
  /**
   * Fetch and BASE64 decode the mc_package_scripts.mcscriptdefinition for the scripts whose
   * mcscriptname is that given in simpleNames and whose mcpackageid is that of the task given by
   * taskId.
   *
   * <p>Internally reads from database tables mc_package_scripts and mxp_tasks.
   *
   * @param simpleNames list of package scripts that only have a name, but no package
   * @param taskId
   * @throws Exception
   */
  protected void fetchScriptSourceFromTaskPackage(
      Connection c, List<PackageScript> simpleNames, int taskId) throws Exception {
    final String M = "fetchScriptSourceFromTaskPackage: ";
    LOG.debug(M + "Entering simpleNames = {}", simpleNames);
    ...
    try {
      ...
      for (int i = 0; i < simpleNames.size(); ++i) {
        ps.setString(2 + i, simpleNames.get(i).getScriptName());
      }
      ...
      while (rs.next()) {
        ...
        LOG.debug(M + "scriptName = {}", scriptName);

        encodedScript = rs.getString(1);
        LOG.debug(M + "encodedScript = {}", encodedScript);
    ...
    LOG.debug(M + "Returning void, simpleNames = {}", simpleNames);
  }

  /**
   * Fetch and BASE64 decode the mc_package_scripts.mcscriptdefinition for the scripts whose
   * mcscriptname is that given in qualifiedNames and whose mcpackageid is that corresponding to the
   * package name given in qualifiedNames.
   *
   * <p>Internally reads from database tables mc_package_scripts and mc_package.
   *
   * @param qualifiedNames list of package scripts that have both a script and package name
   * @throws Exception
   */
  protected void fetchScriptSourceFromNamedPackages(
      Connection c, List<PackageScript> qualifiedNames) throws Exception {
    final String M = "fetchScriptSourceFromNamedPackages: ";
    LOG.debug(M + "Entering qualifiedNames = {}", qualifiedNames);
    ...
    try {
      ...
      while (rs.next()) {
        ...
        LOG.debug(M + "packageName = {}", packageName);
    ...
    LOG.debug(M + "Returning void, qualifiedNames = {}", qualifiedNames);
  }

  public void fetchScriptSource(List<PackageScript> packageScripts, int taskId) throws Exception {
    final String M = "fetchScriptSource: ";
    LOG.debug(M + "Entering packageScripts = {}", packageScripts);
    if (packageScripts != null && !packageScripts.isEmpty()) {
      List<PackageScript> qualifiedNames =
          new java.util.ArrayList<PackageScript>(packageScripts.size());
      List<PackageScript> simpleNames =
          new java.util.ArrayList<PackageScript>(packageScripts.size());
    ...
    LOG.debug(M + "Returning void, packageScripts = {}", packageScripts);
  }

  public List<PackageScript> getScriptNamesOfTask(Object task, String eventName) throws Exception {
    ...
    String mainScriptName =
        (String)
            task.getClass()
                ...
                .invoke(task, new Object[] {eventName});

    LOG.debug(M + "Main script: {} = {}", eventName, mainScriptName);

    if (mainScriptName != null) {
      result.add(new PackageScript(mainScriptName));
    } else {
      throw new ErrorException("Missing task parameter " + eventName);
    }
    ...
    for (int i = 0; i < 10 || haveMoreParameters; ++i) {
      ...
      String requiredScriptParameterValue =
          (String)
              task.getClass()
                  .getMethod("getParameter", new Class<?>[] {String.class})
                  .invoke(task, new Object[] {requiredScriptParameterName});
      if (requiredScriptParameterValue != null) {
        LOG.debug(
            M + "Required scripts: {} = {}",
            requiredScriptParameterName,
            requiredScriptParameterValue);
        result.add(new PackageScript(requiredScriptParameterValue));
      } else {
