...
public class PackageScript {
  ...
  public PackageScript(String qualifiedName) {
    if (qualifiedName == null) {
      throw new ErrorException("Package script qualified name must not be null");
    }

    if (qualifiedName == "") {
      throw new ErrorException("Package script qualified name must not be empty");
    }
    ...
    if (separatorIndex != -1) {
      m_packageName = qualifiedName.substring(0, separatorIndex);

      // Disallow leading and trailing separator
      if (separatorIndex > 0 && separatorIndex < qualifiedName.length() - 1) {
        m_scriptName = qualifiedName.substring(separatorIndex + 1);
      } else {
        throw new ErrorException(qualifiedName + " is not a valid package script qualifiedName.");
      }
    } else {
      m_packageName = null;
  ...
  public boolean equals(Object o) {
    if (o instanceof PackageScript) {
      ...
      return other.m_qualifiedName != null && other.m_qualifiedName.equals(m_qualifiedName);
    } else {
      return false;
    }
  }
